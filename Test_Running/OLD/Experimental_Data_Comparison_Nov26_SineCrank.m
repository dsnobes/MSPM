...
modelCrank = 'Raphael_26Nov2021_from05NovVersion_Sensors_CrankMotion_Nodes748'; % Name of MSPM model geometry.
...
for i = 1:size(RD_DATA,2)
    ...
    RunConditions(i).Model = modelCrank;
    ...
    RunConditions(i).title = ['Crank_' RD_DATA(i).filename];
end

% For sinusoid model
a = length(RunConditions);
for i = 1:size(RD_DATA,2)
    RunConditions(i+a) = RunConditions(end);
    RunConditions(i+a).Model = modelSine;
    RunConditions(i+a).rpm = RD_DATA(i).MB_speed;
    %     RTD_0 --> Displacer Cylinder Head Inlet
    %     RTD_1 --> Displacer Cylinder Head Outlet
    %     RTD_2 --> Heater Inlet
    %     RTD_3 --> Heater Outlet
    %     RTD_4 --> Cooler Inlet
    %     RTD_5 --> Cooler Outlet
    RunConditions(i+a).SourceTemp = mean([mean(RD_DATA(i).RTD_2), mean(RD_DATA(i).RTD_3)]) + 273.15; % K to Celsius
    RunConditions(i+a).SinkTemp = mean([mean(RD_DATA(i).RTD_4), mean(RD_DATA(i).RTD_5)]) + 273.15;
    RunConditions(i+a).EnginePressure = RD_DATA(i).pmean;
    %    RunConditions(i).title = ['Experimental Data Comparison, rpm- ' num2str(RD_DATA(i).MB_speed) ' Pmean- ' num2str(RD_DATA(i).pmean) ' SourceTemp- ' num2str(RD_DATA(i).hot_bath_setpoint) ' SinkTemp- ' num2str(RD_DATA(i).cold_bath_setpoint)];
    RunConditions(i+a).title = ['Sine_' RD_DATA(i).filename];
end
