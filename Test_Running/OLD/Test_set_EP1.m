...
runs(5 + 5 + 5 + 4 + 4 + 4 + 4) = struct(...
  ...
  'rpm',60,... [rpm]
  'max_dt',0.1,... [s]
	'SourceTemp',90 + 273.15 - 2,... [K]
  'SinkTemp',5 + 273.15 + 2,... [K]
...
%}
StrSpeeds = {'0,555767', '0,905133', '1,19915', '1,582467', '2,0'};
NumSpeeds = [33.346, 54.308, 71.949, 94.948, 120];
...
NumSpeeds = [12, 24, 34.917, 52.905, 57.351, 62.669, ...
  ...
  98.664, 109.588, 120];
%}
StrSpeeds = {'0,5820', '0,8818', '1,2407', '1,6444', '2'};
NumSpeeds = [34.917, 52.905, 74.439, 98.664, 120];
for i = 1:length(StrSpeeds)
...
NumSpeeds = [12, 24, 36, 48, 60, 66.329, 83.747, ...
  ...
  135.637, 144];
%}
StrSpeeds = {'0,5', '1,1055', '1,5257', '1,8735', '2,2606'};
NumSpeeds = [30, 66.329, 91.542, 112.408, 135.637];
for i = 1:length(StrSpeeds)
  runs(i+offset).Model = 'EP_1 0,09 DP e0 PP e0';
  runs(i+offset).title = ['90_5, atmos, ' StrSpeeds{i} ...
    ' hz, EP-1 0,09 DP e0 PP e0'];
  runs(i+offset).rpm = NumSpeeds(i);
end

offset = offset + length(StrSpeeds);
StrSpeeds = {'0,5', '1,0', '1,5', '2,0'};
NumSpeeds = [30, 60, 90, 120];
for i = 1:length(StrSpeeds)
