...
runs(4 + 4 + 4) = struct(...
  ...
  'rpm',60,... [rpm]
  'max_dt',0.1,... [s]
	'SourceTemp',90 + 273.15 - 4,... [K]
  'SinkTemp',5 + 273.15 + 4,... [K]
  'EnginePressure',92892, ...
...
%}
StrSpeeds = {'0,555767', '0,905133', '1,19915', '1,582467'};
NumSpeeds = [33.346, 54.308, 71.949, 94.948];

for i = 1:length(StrSpeeds)
  runs(i).Model = 'EP_1 0,09 DP e1_5B PP e1_5B - DV';
  runs(i).title = ['90_5, atmos, ' StrSpeeds{i} ...
    ' hz, EP-1 0,09 DP e1_5B PP e1_5B - DV'];
...
NumSpeeds = [12, 24, 34.917, 52.905, 57.351, 62.669, ...
  ...
  98.664, 109.588, 120];
%}
StrSpeeds = {'0,5820', '0,8818', '1,2407', '1,6444'};
NumSpeeds = [34.917, 52.905, 74.439, 98.664];
for i = 1:length(StrSpeeds)
  runs(i+offset).Model = 'EP_1 0,09 DP e1_5B PP e0 - DV';
  runs(i+offset).title = ['90_5, atmos, ' StrSpeeds{i} ...
    ' hz, EP-1 0,09 DP e1_5B PP e0 - DV'];
...
for i = 1:length(StrSpeeds)
  runs(i+offset).Model = 'EP_1 0,09 DP e0 PP e0 - DV';
  runs(i+offset).title = ['90_5, atmos, ' StrSpeeds{i} ...
    ' hz, EP-1 0,09 DP e0 PP e0 - DV'];
  runs(i+offset).rpm = NumSpeeds(i);
end
%{
offset = offset + length(StrSpeeds);
StrSpeeds = {'0,2', '0,5', '1,0', '1,5', '2,0', '2,5', '3,0'};
NumSpeeds = [12, 30, 60, 90, 120, 150, 180];
for i = 1:length(StrSpeeds)
...
offset = offset + length(StrSpeeds);
StrSpeeds = {'0,5', '1,0', '1,5', '2,0', '2,5', '3,0'};
NumSpeeds = [30, 60, 90, 120, 150, 180];
for i = 1:length(StrSpeeds)
