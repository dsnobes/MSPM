...
for d = 1:n
    ...
    % Load sensor data for all point sensors (1D data). Does not deal with line sensors.
    % Loads files that match multiple queries (defined at top)
    Sfiles = struct([]);
    for q = 1:length(query_sensor)
        Sfiles = [Sfiles; dir(fullfile(this_folderpath, query_sensor{q}))];
    end
    nS = length(Sfiles);
...
%     haveS = 0;
    ...
    if havePP; PV_PP.Wind = trapz(V_closed(:,PV_order(3)), p_closed(:,PV_order(3))); end
    ...
    if haveCC && havePP; FW = FW_Subfunction_v4(PV_PP.p, PV_CC.p, PV_PP.V); end
