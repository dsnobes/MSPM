      ...
      if do_warmup
        ...
        function implicitSolve(ME,initialize) %#ok<INUSD>
            ...
            if nargin == 2
                ...
                Fcu = zeros(lenf,1); Fcrho = Fcu; Fcmu = Fcu;
                FcT = Fcu; FcP = Fcu; theta_FL = Fcu;
            ...
            while ~done
                ...
                %         Fcu = Fcu + factor.*(ME.Fc_A.*ui(nd0) + ME.Fc_B.*ui(nd1) + ...
                ...
                for i = 1:n
                    ...
                    Fcrho(faces) = FcP(faces)./(FcT(faces).*ME.R(i));
                end
                ...
                % Boundary Work, FLow Work and Heat Transport
                P0 = zeros(length(ME.Regions),1);
                polyindex = zeros(size(BW));
                for i = 1:length(ME.Regions)
                    nodes = ME.Regions{i};
                    if ME.isEnvironmentRegion(i)
                        P0(i) = PEnv;
                    ...
                    for ni = nodes(nodes~=lenn)
                        data = ME.Faces{ni,1};
                        for p = 1:size(data,1)
                            fc = data(p,1);
                            y = double(data(p,2));
                            FW(ni) = FW(ni) + y.*ME.Fc_V(fc).*P0(i);
                            QT(ni) = QT(ni) + y.*ME.Fc_V(fc).*Fcrho(fc).*Fcu(fc);
                        end
                    end
                end
                ...
                not_done = true;
                gamma = ME.dT_dU(indn).*ME.dh_dT(indn);
                q_w = (Q(indn) + QT(indn) + Qtd(indn)./h)./(FW(indn) + BW(indn));
                polyindex = (1-gamma).*q_w + gamma;% (1-y)*dq/dw + y
                polyindex(polyindex > 1e8) = 1e8;
                polyindex(end) = 1e-8;
                new_polyindex = polyindex;
                while not_done
                    %% Fine Features Approximation
                    i = 1;
                    Qi = Q(ME.Fc_Nd(fcs,1));
                    ...
                    Cn = (gamma - 1).*(polyindex - 1)./(ME.Rs(indn).*(polyindex - gamma));
                        ...
                        if ~isempty(ME.RegionFcs{i})
                            ...
                            if ME.isEnvironmentRegion(i)
                                ...
                                for row = 1:length(faces)
                                    ni = nodes(row);
                                    b(row) = Vnew(ni)*PEnv/ME.R(i) - Ti(ni)*mi(ni) - Cn(ni).*(h.*Q(indn) + Qtd(indn));
                                    data = ME.Faces{ni,1};
                                    for p = 1:size(data,1)
                                        fc = data(p,1);
                                        y = double(data(p,2));
                                        X = h*y*(Ti(ni) + Cn(ni).*Fcu(fc))/Vnew(ni);
                                ...
                                for row = 1:length(faces)
                                    ...
                                    nj = nd2(faces(row));
                                    b(row) = ...
                                        (Ti(ni)*mi(ni) + Cn(ni).*(h*Q(ni) + Qtd(ni)))/Vnew(ni) - ...
                                        (Ti(nj)*mi(nj) + Cn(nj).*(h*Q(nj) + Qtd(nj)))/Vnew(nj);
                                    data = ME.Faces{ni,1};
                                    for p = 1:size(data,1)
                                        fc = data(p,1);
                                        y = double(data(p,2));
                                    ...
                                    for p = 1:size(data,1)
                                        fc = data(p,1);
                                        y = double(data(p,2));
                                        X = h*y*(Ti(nj) + Cn(nj)*Fcu(fc))/Vnew(nj);
                                        if data(p,3); b(row) = b(row) - ME.Fc_V(fc)*X;
                                        else; A(row,F2C(fc)) = A(row,F2C(fc)) + X;
                                        end
                                    end
                    ...
                    q_w = (Q(indn) + QT(indn) + Qtd(indn)./h)./(FW(indn) + BW(indn));
                    new_polyindex = (1-gamma).*q_w + gamma;% (1-y)*dq/dw + y
                    %           new_polyindex = ... (1-y)*dq/dw + y
                    %             (1-gamma).*(Q(indn) + QT(indn) + Qtd(indn)./h)./...
                    %             (FW(indn) + BW(indn)) + gamma;
                    new_polyindex(new_polyindex < 0) = 0;
                    new_polyindex(new_polyindex > 1e8) = 1e8;
                    new_polyindex(end) = 1e-8;
                    if 1/(sum(1/(abs(new_polyindex-polyindex) + 1e-8)) + 1)/lenn > 0.02
                        not_done = true;
                        polyindex = 0.5*polyindex + 0.5*new_polyindex;
                    end
                    if ~isempty(ME.Fc_dx)
                ...
                for i = 1:n
                    ...
                    Tnew(nodes) = Ti(nodes) + Cn(nodes).*(h.*(Q(nodes) + QT(nodes)) + Qtd(nodes))./mnew(nodes);
                    unew(nodes) = (Tnew(nodes) - Ti(nodes))./ME.dT_dU(nodes);
