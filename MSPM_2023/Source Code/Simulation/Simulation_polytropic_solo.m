      ...
      if do_warmup
        ...
        function implicitSolve(ME,initialize) %#ok<INUSD>
            ...
            while ~done
                ...
                for i = 1:length(ME.Regions)
                    nodes = ME.Regions{i};
                    region_gamma(i) = sum(gamma(nodes).*mi(nodes))./sum(mi(nodes));
                    if ME.isEnvironmentRegion(i)
                        P0(i) = PEnv;
                        %             polyindex(i) = 0;
                    else
                ...
                while not_done
                    ...
                    for i = 1:length(ME.Regions)
                        ...
                        nodes = ME.Regions{i};
                        if ME.isEnvironmentRegion(i)
                            nodes = nodes(nodes~=lenn);
                            polyindex(i) = 0;
                        else
                            q_w(i) = (sum(Q(nodes)) + sum(QT(nodes)) + sum(Qtd(nodes))./h)./...
                                (sum(FW(nodes)) + sum(BW(nodes)));
                            polyindex(i) = (1-region_gamma(i)).*q_w(i) + region_gamma(i);% (1-y)*dq/dw + y
                        end
                        Cn(i) = (region_gamma(i) - 1).*(polyindex(i) - 1)./(ME.R(i).*(polyindex(i) - region_gamma(i)));
                    end
                        ...
                        if ~isempty(ME.RegionFcs{i})
                            ...
                            if ME.isEnvironmentRegion(i)
                                ...
                                for row = 1:length(faces)
                                    ni = nodes(row);
                                    b(row) = Vnew(ni)*PEnv/ME.R(i) - Ti(ni)*mi(ni) - Cn(i).*(h.*Q(indn) + Qtd(indn));
                                    data = ME.Faces{ni,1};
                                    for p = 1:size(data,1)
                                        fc = data(p,1);
                                        y = double(data(p,2));
                                        X = h*y*(Ti(ni) + Cn(i).*Fcu(fc))/Vnew(ni);
                                ...
                                for row = 1:length(faces)
                                    ...
                                    nj = nd2(faces(row));
                                    b(row) = ...
                                        (Ti(ni)*mi(ni) + Cn(i).*(h*Q(ni) + Qtd(ni)))/Vnew(ni) - ...
                                        (Ti(nj)*mi(nj) + Cn(i).*(h*Q(nj) + Qtd(nj)))/Vnew(nj);
                                    data = ME.Faces{ni,1};
                                    for p = 1:size(data,1)
                                        fc = data(p,1);
                                        y = double(data(p,2));
                                    ...
                                    for p = 1:size(data,1)
                                        fc = data(p,1);
                                        y = double(data(p,2));
                                        X = h*y*(Ti(nj) + Cn(i)*Fcu(fc))/Vnew(nj);
                                        if data(p,3); b(row) = b(row) - ME.Fc_V(fc)*X;
                                        else; A(row,F2C(fc)) = A(row,F2C(fc)) + X;
                                        end
                                    end
                ...
                for i = 1:n
                    ...
                    Tnew(nodes) = Ti(nodes) + Cn(i).*(h.*(Q(nodes) + QT(nodes)) + Qtd(nodes))./mnew(nodes);
